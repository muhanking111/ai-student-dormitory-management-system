...
> 状态：`SUPERSEDED`。该不可移动候选在公开后的 GitHub CodeQL 分析中出现 5 个未关闭告警，已由 `rc-20260811.2` 替代；不得再作为当前封版或安全结论。项目许可证为 `Apache-2.0`，本文不表示预发布或生产放行。

## 失效原因

- CodeQL run `31477640111` 发现 1 个 critical 与 4 个 high 告警，涉及分页窄化强转、PII 正则回溯和前端测试正则。
- 告警未被无依据 dismiss；修复后从新源码重新执行完整 Route A 工程门，并生成不可移动新 tag `rc-20260811.2`。
- 本 tag 与 Release 保留用于审计，不移动、不删除、不复用。
