...
const visualReferenceInstantIso = visualReferenceInstant.toISOString()
const visualReferenceEpochSeconds = Math.floor(visualReferenceInstant.getTime() / 1_000)
const visualDatabaseURL = (() => {
  const value = visualBackendEnv.DB_URL?.trim()
  if (!value) throw new Error('正式视觉套件缺少隔离 MySQL DB_URL')
  if (/[?&]sessionVariables=/i.test(value)) {
    throw new Error('正式视觉套件拒绝覆盖既有 MySQL sessionVariables')
  }
  const separator = value.includes('?')
    ? (value.endsWith('?') || value.endsWith('&') ? '' : '&')
    : '?'
  return `${value}${separator}sessionVariables=timestamp=${visualReferenceEpochSeconds}`
})()
const visualAiIsolation = requireAiLiveIsolation(
  visualBackendEnv,
...
const visualAiBackendEnv = {
  ...
  SPRING_DATASOURCE_URL: visualDatabaseURL,
  ...
  DORMITORY_AI_BUSINESS_CLOCK_FIXED_INSTANT: visualReferenceInstantIso,
  DORMITORY_AI_BUSINESS_CLOCK_ZONE: 'Asia/Shanghai',
  JAVA_TOOL_OPTIONS: visualJavaToolOptions,
}

export default defineConfig({
  testDir: './e2e',
  testMatch: 'live-visual.spec.ts',
